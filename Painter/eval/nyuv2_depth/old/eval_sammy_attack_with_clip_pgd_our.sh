...
# inference
CUDA_VISIBLE_DEVICES=6 python eval/nyuv2_depth/painter_inference_depth_attack_with_clip_our.py \
  --ckpt_path /hhd3/ld/checkpoint/ckpt_Painter/painter_vit_large.pth \
  --model ${MODEL} \
  --prompt ${PROMPT} \
  --attack_id ${ATTACK_ID} \
...
########### bash eval/nyuv2_depth/eval_sammy_attack_with_clip_pgd_our.sh 