...
if __name__ == '__main__':
    ...
    ## 108: /data1/; 110: /hhd3/
    dst_dir = os.path.join(f'/hhd3/ld/data/nyu_depth_v2/output_attack/{args.attack_method}_{args.step_size}/'
    ...
    save_data_path = f'/hhd3/ld/data/Painter_root/nyu_depth/attack_analysis/{args.attack_id}_{args.epsilon}/'
    ...
    for img_path in tqdm.tqdm(img_path_list):
        ...
        adv_img, adv_tgt, loss_list = get_adv_img_adv_tgt(img, tgt, model_painter, device, args.attack_id, args.attack_method, epsilon=args.epsilon, num_steps=args.num_steps)
        if i <= 67:
            # np.save(f'{save_data_path}/img2_prompt_67.npy', img)
            # np.save(f'{save_data_path}/img2_prompt_adv_67.npy', adv_img)
            # np.save(f'{save_data_path}/tgt2_prompt_67.npy', tgt)
            # np.save(f'{save_data_path}/tgt2_prompt_adv_67.npy', adv_tgt)
            loss_for_dict[f'{i}'] = loss_list
        else:
            np_dict = np.array(list(loss_for_dict.items()))
            np.save(f'{save_data_path}/loss_for_dict.npy', np_dict)

            ### load this file
            # loaded_np_dict = np.load("my_dict.npy", allow_pickle=True)

            # # Convert the NumPy array back to a dictionary
            # loaded_dict = dict(loaded_np_dict)
        i += 1
