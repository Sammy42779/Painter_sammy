...
def construct_adv_C_pgd_breakAC_feat(img, tgt, model, device, epsilon, num_steps, step_size, rand_init='rand', lam=1, woB=False):
    ...
    for i in range(num_steps): 
        ...
        elif not woB:  # True 那就是不保留B图, 输入的时候B图为mask掉的内容
...
def get_adv_img_adv_tgt_breakAC_feat(img, tgt, model_painter, device, attack_id, attack_method, epsilon, num_steps, lam):
    if attack_id == 'attack_C':
        if attack_method == 'PGD':
            adv_img, adv_tgt = construct_adv_C_pgd_breakAC_feat(img, tgt, model_painter, device, 
