...
def construct_adv_A_pgd_breakAC_feat(img, tgt, model, device, epsilon, num_steps, step_size, rand_init='rand'):

    # print('construct_adv_A_pgd_breakAC_feat')
    ...
    # with torch.no_grad():
    #     clean_a, clean_c = model.forward_encoder_ac(images_normalize(x_adv).float().to(device), images_normalize(tgt).float().to(device), bool_masked_pos.to(device))
    #     print(type(clean_a), type(clean_c))
    #     clean_a = torch.cat(clean_a, dim=-1)
    #     clean_c = torch.cat(clean_c, dim=-1)
    ...
    for _ in range(num_steps):   
        ...
        adv_a, clean_c = model.forward_encoder_ac(images_normalize(x_adv).float().to(device), images_normalize(tgt).float().to(device), bool_masked_pos.to(device))
        # print(adv_a.shape, clean_c.shape)

        loss_ac = L2(adv_a, clean_c) * 0.1
        # loss_ac = criterion(F.log_softmax(adv_a, dim=-1), F.softmax(clean_c, dim=-1)) * 0.0001
...
def construct_adv_C_pgd_breakAC_feat(img, tgt, model, device, epsilon, num_steps, step_size, rand_init='rand', lam=1):

    # print('construct_adv_C_pgd_breakAC_feat')
...
def get_adv_img_adv_tgt_breakAC_feat(img, tgt, model_painter, device, attack_id, attack_method, epsilon, num_steps, lam):
    if attack_id == 'attack_A':
        if attack_method == 'FGSM':
            ...
            adv_img, adv_tgt = construct_adv_A_pgd_breakAC_feat(img, tgt, model_painter, device, epsilon=epsilon/255., num_steps=num_steps, step_size=2/255, rand_init='rand')
        ...
        if attack_method == 'FGSM':
            ...
            adv_img, adv_tgt = construct_adv_C_pgd_breakAC_feat(img, tgt, model_painter, device, epsilon=epsilon/255., num_steps=num_steps, step_size=2/255, rand_init='rand', lam=lam)
