...
def construct_adv_C_pgd_ours_l2_only_c(img, tgt, model, device, epsilon, num_steps, step_size, rand_init='rand'):
...
def get_adv_img_adv_tgt_ours_l2(img, tgt, model_painter, device, attack_id, attack_method, epsilon, num_steps):
    ...
    elif attack_id == 'attack_C_only_c':
        if attack_method == 'PGD':
            adv_img, adv_tgt = construct_adv_C_pgd_ours_l2_only_c(img, tgt, model_painter, device, epsilon=epsilon/255., num_steps=num_steps, step_size=2/255, rand_init='rand')
