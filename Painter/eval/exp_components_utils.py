...
NYU_LIST_B_sync = glob.glob('/hhd3/ld/data/nyu_depth_v2/sync/*/sync_depth*.png')
...
def load_origin_tgt(tgt_path, input_size, task=None):
    ...
    elif task in ['ade20k_segment', 'lol_enhance']:
        tgt2 = Image.open(tgt_path)
        tgt2 = tgt2.resize((input_size, input_size))
        tgt2 = np.array(tgt2) / 255. 
    else:
...
def get_prompt_gt(img2_path, tgt2_path, input_size, exp_id, transfer_img=None, task=None):
    ...
    ########## BASELINE ##########  
    if exp_id == 'baseline': ## baseline
        img2 = load_origin_img(img2_path, input_size)
        tgt2 = load_origin_tgt(tgt2_path, input_size, task=task)

    elif exp_id == 'POS_A_random_A_other_task_coco_gt':
        img2 = load_origin_img(random.choice(COCO_LIST_B), input_size)
        tgt2 = load_origin_tgt(tgt2_path, input_size, task=task)

    ########## MASK A ##########    
    elif exp_id == 'POS_A_mask_A': ## mask_A
        print('@@@@@@ mask A @@@@@@')
    ...
    ########## MASK B ##########
    elif exp_id == 'POS_B_mask_B': ## mask_B
        print('@@@@@@ mask B @@@@@@')
    ...
    ########## MASK A & B ##########
    elif exp_id == 'POS_AB_mask_AB': ## mask_AB
    ...
    ########## EXCHANGE A & B ##########
    elif exp_id == 'POS_AB_exchange_AB': ## exchange_AB
        print('@@@@@@ exchange AB @@@@@@')
        tgt2 = load_origin_img(img2_path, input_size)
        img2 = load_origin_tgt(tgt2_path, input_size, task=task)

    ########## RANDOM A & SAME TASK ##########
    elif exp_id == 'POS_A_random_A_same_task': ## random_A_same_task
        ...
        elif task == 'lol_enhance':
            img2 = load_origin_img(random.choice(LOL_LIST_A), input_size)
        tgt2 = load_origin_tgt(tgt2_path, input_size, task=task)

    ########## RANDOM B & SAME TASK ##########
    elif exp_id == 'POS_B_random_B_same_task': ## random_B_same_task
        ...
        if task == 'nyu_depth':
            ...
            tgt2 = load_origin_tgt(random.choice(LOL_LIST_B), input_size, task=task)

    ########## RANDOM A & OTHER TASK ##########
    elif exp_id == 'POS_A_random_A_other_task': ## random_A_other_task
        ### 将图A替换为COCO_A图
        img2 = load_origin_img(random.choice(COCO_LIST_A), input_size)
        tgt2 = load_origin_tgt(tgt2_path, input_size, task=task)

    ########## RANDOM B & OTHER TASK ##########
    elif exp_id == 'POS_B_random_B_other_task': ## random_B_other_task
        ### 将图B替换为COCO_B图
        img2 = load_origin_img(img2_path, input_size)
    ...
    elif exp_id == 'POS_B_random_B_other_task_random':
        # 将图B替换为其他任务随机ground-truth, 不是prompt原始配对的gt
        img2 = load_origin_img(img2_path, input_size)
