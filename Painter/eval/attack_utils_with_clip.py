...
def construct_adv_AB_pgd_origin(img, tgt, model, device, epsilon, num_steps, step_size, rand_init='rand', is_ours=False):
...
def construct_adv_AB_pgd(img, tgt, model, device, epsilon, num_steps, step_size, rand_init='rand', is_ours=False):
    ...
    print('construct_adv_AB_pgd_ours')
    ...
    for i in range(num_steps): 
        ...
        if is_ours:
            # A' B C
            a_adv_b_c_latent = model.forward_encoder(images_normalize(x_adv).float().to(device), images_normalize(tgt).float().to(device), bool_masked_pos.to(device))
            a_adv_b_c_pred = model.forward_decoder(a_adv_b_c_latent)
            # a_adv_b_c_latent
            # A B' C
            # a_b_adv_c_latent = model.forward_encoder(images_normalize(x).float().to(device), images_normalize(tgt_adv).float().to(device), bool_masked_pos.to(device))
            # a_b_adv_c_pred = model.forward_decoder(a_b_adv_c_latent)

            loss_a_adv_b_c = model.forward_loss(a_adv_b_c_pred, images_normalize(tgt).float().to(device), bool_masked_pos.to(device), valid.to(device))
            # loss_a_b_adv_c = model.forward_loss(a_b_adv_c_pred, images_normalize(tgt).float().to(device), bool_masked_pos.to(device), valid.to(device))

            # loss = loss1 + loss_a_adv_b_c + loss_a_b_adv_c
            loss = loss1 + loss_a_adv_b_c
