...
# bash eval/ade20k_semantic/eval_sammy_with_clip.sh

set -x

PY_FILE=eval/ade20k_semantic/painter_inference_segm_with_clip.py
...
# inference
CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.launch --nproc_per_node=${NUM_GPUS} --master_port=2000 --use_env \
   ${PY_FILE}\
