...
def run_one_image(img, tgt, size, model, out_path, device):
    ...
    latent = model.module.forward_encoder(images_normalize(x).float().to(device), images_normalize(tgt).float().to(device), bool_masked_pos.to(device))  ## 得到encoder后的输出, 4个层的输出, 每一个输出的shape是[1,56,28,1024]
    pred = model.module.forward_decoder(latent)  # [N, L, p*p*3]
    loss = model.module.forward_loss(pred, images_normalize(tgt).float().to(device), bool_masked_pos.to(device), valid.to(device))
    y = model.module.patchify(pred)
