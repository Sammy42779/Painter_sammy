...
def construct_adv_A_pgd_ours_breakAC_with_B(img, tgt, model, device, epsilon, num_steps, step_size, rand_init='rand'):
    ...
    with torch.no_grad():
        clean_c = x.detach().clone() * pos_c
        bc_latent = model.forward_encoder(images_normalize(clean_c).float().to(device), images_normalize(tgt).float().to(device), bool_masked_pos.to(device))
        bc_latent = torch.cat(bc_latent, dim=-1)
    ...
    for _ in range(num_steps): 
        ...
        ab = x_adv.detach().clone() * pos_a
        ab_latent = model.forward_encoder(images_normalize(ab).float().to(device), images_normalize(tgt).float().to(device), bool_masked_pos.to(device))
        ab_latent = torch.cat(ab_latent, dim=-1)

        ac_loss = L2(ab_latent, bc_latent)
...
def get_adv_img_adv_tgt_ours_breakAC_with_B(img, tgt, model_painter, device, attack_id, attack_method, epsilon, num_steps):
    if attack_id == 'attack_A':
        if attack_method == 'FGSM':
            ...
            adv_img, adv_tgt = construct_adv_A_pgd_ours_breakAC_with_B(img, tgt, model_painter, device, epsilon=epsilon/255., num_steps=num_steps, step_size=2/255, rand_init='rand')
