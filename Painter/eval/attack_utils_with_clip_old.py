...
def construct_adv_C_pgd(img, tgt, model, device, epsilon, num_steps, step_size, rand_init='rand', is_ours=False):
...
def construct_adv_AB_pgd1(img, tgt, model, device, epsilon, num_steps, step_size, rand_init='rand', is_ours=False):
    ...
    if is_ours:
        with torch.no_grad():
            c = x.detach() * c_mask   ## 整张图只保留左下角的C
            c_latent = model.forward_encoder(images_normalize(c).float().to(device), images_normalize(mask_tgt).float().to(device), bool_masked_pos.float().to(device))
            c_latent = torch.cat(c_latent, dim=-1)  # [1,56,28,1024]
    ...
    for i in range(num_steps): 
        ...
        if is_ours:
            a_adv = x_adv.detach() * mask_x   ## 整张图只保留左上角的A_adv
            a_adv_latent = model.forward_encoder(images_normalize(a_adv).float().to(device), images_normalize(mask_tgt).float().to(device), bool_masked_pos.to(device))
            a_adv_latent = torch.cat(a_adv_latent, dim=-1)

            l2_loss = L2(a_adv_latent, c_latent)
...
def construct_adv_AC_pgd(img, tgt, model, device, epsilon, num_steps, step_size, rand_init='rand', is_ours=False):
    ...
    for i in range(num_steps): 
        ...
        if is_ours:
            a_adv = x_adv.detach() * pos_a   ## 整张图只保留左上角的A_adv
            a_adv_latent = model.forward_encoder(images_normalize(a_adv).float().to(device), images_normalize(mask_tgt).float().to(device), bool_masked_pos.to(device))
            a_adv_latent = torch.cat(a_adv_latent, dim=-1)
            a_adv_latent = mean_pool(a_adv_latent)

            c_adv = x_adv.detach() * pos_c   ## 整张图只保留左下角的C_adv
            c_adv_latent = model.forward_encoder(images_normalize(c_adv).float().to(device), images_normalize(mask_tgt).float().to(device), bool_masked_pos.to(device))
            c_adv_latent = torch.cat(c_adv_latent, dim=-1)
            c_adv_latent = mean_pool(c_adv_latent)

            l2_loss = L2(a_adv_latent, c_adv_latent)
...
def construct_adv_BC_pgd(img, tgt, model, device, epsilon, num_steps, step_size, rand_init='rand', is_ours=False):
...
def construct_adv_ABC_pgd(img, tgt, model, device, epsilon, num_steps, step_size, rand_init='rand', is_ours=False):
...
def get_adv_img_adv_tgt(img, tgt, model_painter, device, attack_id, attack_method, epsilon, num_steps, is_ours=False):
    if attack_id == 'attack_A':
        ...
        if attack_method == 'PGD':
            adv_img, adv_tgt = construct_adv_BC_pgd(img, tgt, model_painter, device, epsilon=epsilon/255., num_steps=num_steps, step_size=2/255, rand_init='rand', is_ours=is_ours)
    elif attack_id == 'attack_ABC':
        if attack_method == 'PGD':
            adv_img, adv_tgt = construct_adv_ABC_pgd(img, tgt, model_painter, device, epsilon=epsilon/255., num_steps=num_steps, step_size=2/255, rand_init='rand', is_ours=is_ours)
    elif attack_id == 'none':
